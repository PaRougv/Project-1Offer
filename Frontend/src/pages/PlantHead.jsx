  ...
  return (
    <main className="management-root">
      ...
      <div className="management-grid">
        ...
        <form className="management-panel" onSubmit={createHod}><h2>New HOD</h2><input placeholder="Full name" value={hod.name} onChange={e => setHod({ ...hod, name: e.target.value })} required /><input type="email" placeholder="Email" value={hod.email} onChange={e => setHod({ ...hod, email: e.target.value })} required /><input type="password" placeholder="Temporary password" value={hod.password} onChange={e => setHod({ ...hod, password: e.target.value })} required /><select value={hod.department} onChange={e => setHod({ ...hod, department: e.target.value })} required><option value="">Choose department</option>{departments.map(item => <option key={item._id} value={item._id}>{item.name}</option>)}</select><button className="management-primary">Create HOD</button></form>
      </div>
      <section className="management-panel"><h2>HODs in this plant</h2>{hods.map(item => <div className="management-row" key={item._id}><strong>{item.name}</strong><span>{item.email} · {item.department?.name || "No department"}</span></div>)}</section>
    </main>
  );
};
